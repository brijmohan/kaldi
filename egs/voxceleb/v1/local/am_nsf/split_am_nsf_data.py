...
print(f"Config: root_dir = {root_dir}, out_dir = {out_dir}")
print(f"Config: ntest = {ntest}, ndev = {ndev}")
...
test_spks = []
spklim = int(ntest / 2)
print(f"Per gender speaker limit for test = {spklim}")
mspk, fspk = 0, 0
...
print(f"Selected {len(test_spks)} test speakers.")

# Find dev spks and utts
dev_spks = []
dev_utts = []
spklim = int(ndev / 2)
print(f"Per gender speaker limit for dev = {spklim}")
...
print(f"Selected {len(dev_spks)} dev speakers.")
...
train_utts = [utt for utt in all_utts if utt2spk[utt] not in test_spks and
                utt not in dev_utts]
print(f"Selected {len(train_spks)} train speakers and {len(train_utts)} train utterances.")
